...
from .pick_place_cfg import PickPlaceEnvCfg, RandomizationCfg


class PickPlaceEnv(IsaacEnv):
    """Environment for picking and placing an object off a table with a single-arm manipulator."""

    def __init__(self, cfg: PickPlaceEnvCfg = None, headless: bool = False):
        # copy configuration
        self.cfg = cfg
        ...
        # object number 1
        self.cfg.object1 = copy.deepcopy(self.cfg.object)
        self.cfg.object1.init_state.pos = (0.5, -0.1, 0.075)
        self.object1 = RigidObject(cfg=self.cfg.object1)
        self.objects = [self.object, self.object1]
        ...
        self._observation_manager = PickPlaceObservationManager(class_to_dict(self.cfg.observations), self, self.device)
        # prepare the reward manager
        self._reward_manager = PickPlaceRewardManager(
...
class PickPlaceObservationManager(ObservationManager):
    """Reward manager for single-arm reaching environment."""

    def arm_dof_pos_scaled(self, env: PickPlaceEnv):
    ...
    def arm_dof_vel(self, env: PickPlaceEnv):
        """DOF velocity of the arm."""
        return env.robot.data.arm_dof_vel

    def tool_dof_pos_scaled(self, env: PickPlaceEnv):
...
class PickPlaceRewardManager(RewardManager):
    ...
    def lifting_object_success(self, env: PickPlaceEnv, threshold: float):
