...
from .chemistry_v1_cfg import ChemistryV1EnvCfg, RandomizationCfg


class ChemistryV1Env(IsaacEnv):
    """Environment for picking and placing an object off a table with a single-arm manipulator."""

    def __init__(self, cfg: ChemistryV1EnvCfg = None, headless: bool = False):
        # copy configuration
        self.cfg = cfg
        ...
        # object number 0
        self.beaker_500ml = RigidObject(cfg=self.cfg.beaker_500ml)
        # object number 1
        self.scale_ika = RigidObject(cfg=self.cfg.scale_ika)
        self.objects = [self.beaker_500ml, self.scale_ika]
        self.object = self.beaker_500ml
        ...
        self._observation_manager = ChemistryV1ObservationManager(class_to_dict(self.cfg.observations), self, self.device)
        # prepare the reward manager
        self._reward_manager = ChemistryV1RewardManager(
    ...
    def _randomize_object_initial_pose(self, env_ids: torch.Tensor, cfg: RandomizationCfg.ObjectInitialPoseCfg):
        ...
        for i in range(len(self.objects)):
            ...
            print()
            print("-------------------------------")
            print("_randomize_object_initial_pose")
            print(f"object[{i}] :", root_state )
            print("-------------------------------")
            print()
...
class ChemistryV1ObservationManager(ObservationManager):
    """Reward manager for single-arm reaching environment."""

    def arm_dof_pos_scaled(self, env: ChemistryV1Env):
    ...
    def arm_dof_vel(self, env: ChemistryV1Env):
        """DOF velocity of the arm."""
        return env.robot.data.arm_dof_vel

    def tool_dof_pos_scaled(self, env: ChemistryV1Env):
    ...
    def tool_positions(self, env: ChemistryV1Env):
        """Current end-effector position of the arm."""
        return env.robot.data.ee_state_w[:, :3] - env.envs_positions

    def object_positions(self, env: ChemistryV1Env):
        """Current object position."""
        return env.object.data.root_pos_w - env.envs_positions

    def object_desired_positions(self, env: ChemistryV1Env):
        """Desired object position."""
        return env.object_des_pose_w[:, 0:3] - env.envs_positions

    def actions(self, env: ChemistryV1Env):
        """Last actions provided to env."""
        return env.actions


class ChemistryV1RewardManager(RewardManager):
    """Reward manager for single-arm object picking and placing environment."""

    def reaching_object_position_l2(self, env: ChemistryV1Env):
    ...
    def lifting_object_success(self, env: ChemistryV1Env, threshold: float):
