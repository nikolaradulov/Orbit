...
from .vial_insertion_cfg import RandomizationCfg, VialInsertionEnvCfg


class VialInsertionEnv(IsaacEnv):
    """Environment for vial insertion into rack with a single-arm manipulator."""

    def __init__(self, cfg: VialInsertionEnvCfg = None, headless: bool = False):
        ...
        self.rack = RigidObject(cfg=self.cfg.rack)
        self.objects = [self.vial, self.rack]
        ...
        self._observation_manager = VialInsertionObservationManager(
            class_to_dict(self.cfg.observations), self, self.device
        )
        # prepare the reward manager
        self._reward_manager = VialInsertionRewardManager(
        ...
        self.sim.step()
        # -- fill up buffers
        self.vial.update_buffers(self.dt)
        self.rack.update_buffers(self.dt)
        self.robot.update_buffers(self.dt)
        # print(f"-------------------------{self.cfg.control.control_type}----------------------------")
        print()
    ...
    def _design_scene(self) -> List[str]:
        ...
        # rack
        self.rack.spawn(self.template_env_ns + "/Rack")
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        object_position_error = torch.norm(self.vial.data.root_pos_w - self.object_des_pose_w[:, 0:3], dim=1)
    ...
    def _initialize_views(self) -> None:
        ...
        self.rack.initialize(self.env_ns + "/.*/Rack")
    ...
    def _check_termination(self) -> None:
        ...
        object_pos = self.vial.data.root_pos_w - self.envs_positions
    ...
    def _randomize_object_initial_pose(self, env_ids: torch.Tensor, cfg: RandomizationCfg.ObjectInitialPoseCfg):
        ...
        root_state = self.vial.get_default_root_state(env_ids)
        print(root_state)
        ...
        # set the root state
        print(root_state)
        self.vial.set_root_state(root_state, env_ids=env_ids)
...
class VialInsertionObservationManager(ObservationManager):
    """Reward manager for single-arm reaching environment."""

    def arm_dof_pos_scaled(self, env: VialInsertionEnv):
    ...
    def arm_dof_vel(self, env: VialInsertionEnv):
        """DOF velocity of the arm."""
        return env.robot.data.arm_dof_vel

    def tool_dof_pos_scaled(self, env: VialInsertionEnv):
    ...
    def tool_positions(self, env: VialInsertionEnv):
        """Current end-effector position of the arm."""
        return env.robot.data.ee_state_w[:, :3] - env.envs_positions

    def object_positions(self, env: VialInsertionEnv):
        """Current object position."""
        return env.vial.data.root_pos_w - env.envs_positions

    def object_desired_positions(self, env: VialInsertionEnv):
        """Desired object position."""
        return env.object_des_pose_w[:, 0:3] - env.envs_positions

    def actions(self, env: VialInsertionEnv):
        """Last actions provided to env."""
        return env.actions


class VialInsertionRewardManager(RewardManager):
    """Reward manager for single-arm object vial insertion environment."""

    def reaching_object_position_l2(self, env: VialInsertionEnv):
        """Penalize end-effector tracking position error using L2-kernel."""
        return torch.sum(torch.square(env.robot.data.ee_state_w[:, 0:3] - env.vial.data.root_pos_w), dim=1)

    def reaching_object_position_exp(self, env: VialInsertionEnv, sigma: float):
        """Penalize end-effector tracking position error using exp-kernel."""
        return torch.sum(torch.square(env.robot.data.ee_state_w[:, 0:3] - env.vial.data.root_pos_w), dim=1)

    def penalizing_robot_dof_velocity_l2(self, env: VialInsertionEnv):
        """Penalize large movements of the robot arm."""
        return -torch.sum(torch.square(env.robot.data.arm_dof_vel), dim=1)

    def penalizing_robot_dof_acceleration_l2(self, env: VialInsertionEnv):
        """Penalize fast movements of the robot arm."""
        return -torch.sum(torch.square(env.robot.data.dof_acc), dim=1)

    def penalizing_action_rate_l2(self, env: VialInsertionEnv):
        """Penalize large variations in action commands."""
        return -torch.sum(torch.square(env.actions - env.previous_actions), dim=1)

    def tracking_object_position_exp(self, env: VialInsertionEnv, sigma: float):
        ...
        current_error = torch.sum(torch.square(env.object_des_pose_w[:, 0:3] - env.vial.data.root_pos_w), dim=1)
        return torch.exp(-current_error / sigma) - torch.exp(-initial_error / sigma)

    def lifting_object_success(self, env: VialInsertionEnv, threshold: float):
        """Sparse reward if object is lifted successfully."""
        error = torch.sum(torch.square(env.vial.data.root_pos_w - env.object_des_pose_w[:, 0:3]), dim=1)
