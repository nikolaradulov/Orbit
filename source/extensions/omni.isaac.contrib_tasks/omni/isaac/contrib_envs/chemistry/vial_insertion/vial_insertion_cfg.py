...
@configclass
class VialObjectCfg(RigidObjectCfg):
    """Properties for the object to manipulate in the scene."""

    meta_info = RigidObjectCfg.MetaInfoCfg(
        usd_path=f"{ASSETS_DATA_DIR}/objects/vials/vial_20ml/vial_20ml_inst.usda",
        scale=(0.9, 0.9, 0.9),
    )
    init_state = RigidObjectCfg.InitialStateCfg(
        pos=(0.65, 0.1, 0.01), rot=(1.0, 0.0, 0.0, 0.0), lin_vel=(0.0, 0.0, 0.0), ang_vel=(0.0, 0.0, 0.0)
    )
    ...
    material_props = RigidObjectCfg.PhysicsMaterialCfg(
        static_friction=0.5, dynamic_friction=0.5, restitution=0.0, prim_path="/physics_material"
    )


@configclass
class RackObjectCfg(RigidObjectCfg):
    """Properties for the object to manipulate in the scene."""

    meta_info = RigidObjectCfg.MetaInfoCfg(
        usd_path=f"{ASSETS_DATA_DIR}/objects/racks/custom_6_wells_rack/custom_6_wells_rack_inst.usda",
        scale=(1, 1, 1),
    )
    init_state = RigidObjectCfg.InitialStateCfg(
        pos=(0.65, -0.15, 0.03), rot=(1.0, 0.0, 0.0, 0.0), lin_vel=(0.0, 0.0, 0.0), ang_vel=(0.0, 0.0, 0.0)
    )
    rigid_props = RigidObjectCfg.RigidBodyPropertiesCfg(
        max_angular_velocity=1000.0,
        max_linear_velocity=1000.0,
...
@configclass
class VialInsertionEnvCfg(IsaacEnvCfg):
    """Configuration for the Lift environment."""

    # General Settings
    env: EnvCfg = EnvCfg(num_envs=1024, env_spacing=2.5, episode_length_s=20.0)
    ...
    robot.rigid_props.disable_gravity = True

    # -- object
    vial: VialObjectCfg = VialObjectCfg()
    # -- rack object
    rack: RackObjectCfg = RackObjectCfg()
    # -- table
