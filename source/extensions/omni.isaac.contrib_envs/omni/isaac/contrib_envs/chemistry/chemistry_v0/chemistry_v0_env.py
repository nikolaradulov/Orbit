...
from .chemistry_v0_cfg import ChemistryV0EnvCfg, RandomizationCfg


class ChemistryV0Env(IsaacEnv):
    """Environment for picking and placing an object off a table with a single-arm manipulator."""

    def __init__(self, cfg: ChemistryV0EnvCfg = None, headless: bool = False):
        # copy configuration
        self.cfg = cfg
        ...
        self._observation_manager = ChemistryV0ObservationManager(class_to_dict(self.cfg.observations), self, self.device)
        # prepare the reward manager
        self._reward_manager = ChemistryV0RewardManager(
...
class ChemistryV0ObservationManager(ObservationManager):
    """Reward manager for single-arm reaching environment."""

    def arm_dof_pos_scaled(self, env: ChemistryV0Env):
    ...
    def arm_dof_vel(self, env: ChemistryV0Env):
        """DOF velocity of the arm."""
        return env.robot.data.arm_dof_vel

    def tool_dof_pos_scaled(self, env: ChemistryV0Env):
...
class ChemistryV0RewardManager(RewardManager):
    ...
    def lifting_object_success(self, env: ChemistryV0Env, threshold: float):
